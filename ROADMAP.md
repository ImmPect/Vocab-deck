...
## На будущее (не входит в основной таймлайн)

### Фаза 4: бэкенд и аккаунты — Python + PostgreSQL
Сейчас весь прогресс живёт в `localStorage` браузера — это просто и бесплатно хостится на GitHub Pages, но привязано к одному устройству. Эта фаза открывает синхронизацию между устройствами и хранение своих списков слов не локально, а на сервере.

**Стек:**
- **Python (FastAPI)** — API-сервер: эндпоинты для регистрации/логина, CRUD по словам, сохранение прогресса
- **PostgreSQL** — три основные таблицы: `users` (аккаунты), `words` (личные и общие колоды), `progress` (статус слова у конкретного пользователя: new / learning / known, дата последнего повтора)
- **Хостинг бэкенда** — GitHub Pages для этого не подходит (только статика), нужен отдельный сервис с бесплатным тарифом: Render, Railway или Fly.io

**Почему не в основном плане:** фазы 1-3 полностью самодостаточны на чистом фронтенде и не должны ждать бэкенд. Разумно взяться за это уже после того, как обучающая логика (транскрипция, диктант, EN⇄RU) обкатана на себе — тогда будет понятно, какие данные реально нужно хранить на сервере, а не проектировать БД вслепую.

Когда будете готовы — можем спроектировать схему БД и API вместе, отдельным заходом.
